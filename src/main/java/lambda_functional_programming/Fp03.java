...
public class Fp03 {
    ...
    //3)Create a method to print the elements after ordering according to their length. (In reverse order)
    ...
    //5)Create a method to sort the elements according to their length then according to their first character.
    public static void printSortedByLengthThenSortedByInitial(List<String> list) {

        list.stream().sorted(Comparator.comparing(String::length).thenComparing(t -> t.charAt(0))).forEach(Utils::printInTheSameLineWithSpace);
    }
    ...
    //9)Create a method to check if the length of all elements are less than 12.
